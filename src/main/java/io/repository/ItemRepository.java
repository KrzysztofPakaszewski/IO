...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    @Query(value = "select item from Item item left join fetch User u on u = item.owner where u.login = ?#{principal.username}",
        countQuery = "select count(item) from Item item left join fetch User u on u = item.owner where u.login = ?#{principal.username}")
